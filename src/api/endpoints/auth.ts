...
export const getCurrentUser = async () => {
  ...
  apiClient.get<UserProfile>('/auth/me').then(res => UserProfileSchema.parse(res.data));
};
// Validate/Refresh Session
export const validateSession = () =>
  apiClient.get<UserProfile>('/auth/session').then(res => UserProfileSchema.parse(res.data));
...
export const validateResetToken = (token: string) =>
  apiClient.get<{ valid: boolean }>(`/auth/validate-reset-token/${token}`).then(res => res.data);

// Reset Password
export const resetPassword = (token: string, data: ResetPassword) =>
  apiClient.post(`/auth/reset-password/${token}`, data).then(res => res.data);
...
// Deactivate Account
export const deactivateAccount = () => apiClient.delete('/auth/deactivate').then(res => res.data);
