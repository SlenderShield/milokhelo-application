...
export const getChatRooms = () =>
  apiClient.get<ChatRoom[]>('/chat/rooms').then(res => ChatRoomSchema.array().parse(res.data));

// Create Chat Room
export const createChatRoom = (data: ChatRoomCreate) =>
  apiClient.post<ChatRoom>('/chat/rooms', data).then(res => ChatRoomSchema.parse(res.data));

// Get Chat Messages
export const getChatMessages = (roomId: string, params?: { limit?: number; before?: string }) =>
  apiClient
