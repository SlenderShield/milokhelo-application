...
export const createTeam = (data: TeamCreate) =>
  apiClient
    ...
    .then((res) => TeamSchema.parse(res.data));

// Get Teams (List/Search)
export const getTeams = (params?: SearchQuery) =>
  apiClient
    .get<Team[]>('/teams', { params })
    .then((res) => TeamSchema.array().parse(res.data));
...
export const joinTeam = (id: string, data?: TeamJoin) =>
  apiClient
    .post<{ message: string }>(`/teams/${id}/join`, data)
    .then((res) => res.data);

// Leave Team
export const leaveTeam = (id: string) =>
  apiClient
    .post<{ message: string }>(`/teams/${id}/leave`)
    .then((res) => res.data);
