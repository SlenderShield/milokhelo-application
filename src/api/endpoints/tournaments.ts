...
export const createTournament = (data: TournamentCreate) =>
  apiClient.post<Tournament>('/tournaments', data).then(res => TournamentSchema.parse(res.data));
...
export const getTournamentById = (id: string) =>
  apiClient.get<Tournament>(`/tournaments/${id}`).then(res => TournamentSchema.parse(res.data));
...
export const joinTournament = (id: string, data: TournamentRegister) =>
  apiClient.post<{ message: string }>(`/tournaments/${id}/join`, data).then(res => res.data);

// Leave Tournament
export const leaveTournament = (id: string, data: TournamentRegister) =>
  apiClient.post<{ message: string }>(`/tournaments/${id}/leave`, data).then(res => res.data);

// Register for Tournament (Legacy)
export const registerForTournament = (id: string, data: TournamentRegister) =>
  apiClient.post<{ message: string }>(`/tournaments/${id}/register`, data).then(res => res.data);
